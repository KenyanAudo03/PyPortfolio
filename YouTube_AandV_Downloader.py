...
def video_downloader():
    ...
    Path(downloads_path).mkdir(parents=True, exist_ok=True)  # Create directory if it doesn't exist
    ...
    def download_video():
        ...
        root.after(0, lambda: status_label.config(text=f"Downloaded: {new_name}"))  # Update GUI in main thread
        root.after(0, lambda: open_button.config(state=tk.NORMAL))  # Enable the open button
...
def open_file():
    ...
    if os_type.lower() == "linux":
        try:
            ...
            status_label.config(text="Error opening file.", fg="red", font=16)
